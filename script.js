...
const ADMIN_EMAIL = 'juvensmervil592@gmail.com';
const ADMIN_PASSWORD = 'juvensadmin';
...
function initTopWallet() {
  const walletEl = document.getElementById('top-wallet');
  if (!walletEl) return;
  const wallet = store.get('wallet', { balance: 0, history: [] });
  walletEl.textContent = `Wallet: ${Number(wallet.balance || 0).toLocaleString('fr-FR')} HTG`;
}
...
function initForms() {
  ...
  document.getElementById('login-form')?.addEventListener('submit', async (e) => {
    ...
    if (isAdminCredentials) {
      store.set('adminSession', { email: ADMIN_EMAIL, loggedInAt: new Date().toISOString() });

      if (window.supabaseClient?.auth?.signInWithPassword) {
        try {
          await window.supabaseClient.auth.signInWithPassword({ email: ADMIN_EMAIL, password: ADMIN_PASSWORD });
        } catch (error) {
          console.warn('Connexion Supabase admin non disponible:', error?.message || error);
        }
      }

      alert('Connexion administrateur réussie. Redirection vers le dashboard admin.');
...
function initAdmin() {
  ...
  const adminSession = store.get('adminSession', null);
  if (!adminSession || adminSession.email !== ADMIN_EMAIL) {
    const main = document.querySelector('main.page');
    if (main) {
      main.innerHTML = `<section class="section"><div class="glass auth-card"><h1>Accès administrateur requis</h1><p>Connectez-vous avec l'email administrateur pour gérer le site en temps réel.</p><a class="btn btn-primary" href="../account.html">Se connecter</a></div></section>`;
    }
    return;
  }
